...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        let header: [u8; 8] = bytes.to_vec().get(0..8).unwrap().try_into()?;
        // check the header is standed header
        if u64::from_be_bytes(header) != u64::from_be_bytes(Png::STANDARD_HEADER) {
            Err(anyhow!(MyError::InvalidPngHearder))
        } else {
            ...
            while start < total_len {
                let chunk_length =
                    u32::from_be_bytes(bytes.to_vec().get(start..start + 4).unwrap().try_into()?);
                let end = start + 4 + 4 + chunk_length as usize + 4;
                let chunk = get_chunk_by_range(bytes, chunk_length, start)?;
...
fn get_chunk_by_range(bytes: &[u8], length: u32, start: usize) -> Result<Chunk> {
    let chunk_type = bytes.get(start + 4..start + 4 + 4).unwrap();
    let chunk_type = ChunkType::from_str(std::str::from_utf8(chunk_type)?)?;
...
mod tests {
    ...
    #[test]
    fn test_valid_from_one_bytes() {
        let chunk_bytes: Vec<u8> = testing_one_chunk()
            .into_iter()
            .flat_map(|chunk| chunk.as_bytes())
            .collect();
    ...
    #[test]
    fn test_valid_from_two_bytes() {
        let chunk_bytes: Vec<u8> = testing_two_chunk()
            .into_iter()
            .flat_map(|chunk| chunk.as_bytes())
            .collect();
